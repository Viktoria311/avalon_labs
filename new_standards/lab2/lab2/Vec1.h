...
public:
	...
	Vec1() = delete;
	Vec1(const T& range_x, const T& range_y); // конструктор с установкой диапазона
	Vec1(const T& range_x, const T& range_y, const std::initializer_list<T>& i_list); // конструктор: диапазон и элементы
		...
		: from(range_x), to(range_y)
{
	...
	std::copy_if(i_list.begin(), i_list.end(), std::back_inserter(vec),
		...
		{ 
			// если элемент входит в диапазон 
			...
			else // если элемент не входит в диапазон
			{
				return false;
			}
...
void Vec1<T>::set_range(const T& x, const T& y)
{
	...
	if (vec.size() != 0) // а если метод вызывать у уже использованного вектора для смены диапазона
	{
		//
